...
const BlockContentItem = (props) => {
    ...
    const sendAuth = async (e) => {
        ...
        if (e.target.dataset.info === 'login') {
            const res = await request(`${state.SERVER.url}/api/auth/login`, 'POST', user)
            setToast(res)
            if (res.tokenAuth) {
                auth.login({token: res.tokenAuth, role: res.role, name: res.name})
            }
        ...
        return (
            <div className={s.blockContentItem}>
                <form onSubmit={sendAuth} data-info="login" className={s.blockContentForm}>
                    <input type="text" placeholder="Login" name="login" onChange={handlerInput} value={input.login}
                           className={s.blockContentFormInput}/>
                    <input type="password" placeholder="Password" name="password" onChange={handlerInput}
                           value={input.password} className={s.blockContentFormInput}/>
                    <input type="submit" value={!loading ? 'Войти' : 'Загрузка'}
                           className={s.blockContentFormSubmit + " " + s.mt}/>
                </form>
            </div>
        )
    } else {
            ...
            <div className={s.blockContentItem}>
                <form onSubmit={sendAuth} data-info="register" className={s.blockContentForm}>
                    <input
                        ...
                        value={input.login}
                    />
                    <input type="email" placeholder="Email" name="email" onChange={handlerInput} value={input.email}
                           className={s.blockContentFormInput}/>
                    <input type="password" placeholder="Password" name="password" onChange={handlerInput}
                           value={input.password} className={s.blockContentFormInput}/>
                    ...
                    <div
                        className={`${s.divclass} ${s.oneclass} ${(!role && !(active1 && !active2) || (!active1 && active2)) ? s.twoclass : ''}`}>
                    ...
                    <input type="submit" value={!loading ? 'Регистрация' : 'Загрузка'}
                           className={s.blockContentFormSubmit}/>
                </form>
            </div>
        )
    }
